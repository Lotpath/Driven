...
namespace Driven.Bootstrapper
{
    ...
    public static class AppDomainAssemblyTypeScanner
    {
        ...
        {
            LoadAssembliesWithDrivenReferences();
        }

        /// <summary>
        /// Driven core assembly
        /// </summary>
        private static Assembly drivenAssembly = typeof(DrivenEngine).Assembly;
        ...
        public static Func<Assembly, bool>[] DefaultAssembliesToScan = new Func<Assembly, bool>[]
        {
            ...
            x =>
            {
                return !x.GetName().Name.StartsWith("Driven.Testing",StringComparison.OrdinalIgnoreCase) &&
                    x.GetReferencedAssemblies().Any(r => r.Name.StartsWith("Driven", StringComparison.OrdinalIgnoreCase));
            }
        };
        ...
        /// <summary>
        /// Add assemblies to the list of assemblies to scan for Driven types
        /// </summary>
        ...
        /// <summary>
        /// Add predicates for determining which assemblies to scan for Driven types
        /// </summary>
        ...
        /// <summary>
        /// Loads any assembly that references a Driven assembly.
        /// </summary>
        public static void LoadAssembliesWithDrivenReferences()
        {
            ...
            foreach (var directory in GetAssemblyDirectories())
            {
                ...
                foreach (var unloadedAssembly in unloadedAssemblies)
                {
                    ...
                    if (inspectedAssembly != null && inspectedAssembly.GetReferencedAssemblies().Any(r => r.Name.StartsWith("Driven", StringComparison.OrdinalIgnoreCase)))
                    {
                        try
                        {
        ...
        public static IEnumerable<Type> TypesOf(Type type, ScanMode mode)
        {
            ...
            return (mode == ScanMode.OnlyDriven) ?
                returnTypes.Where(t => t.Assembly == drivenAssembly) :
                returnTypes.Where(t => t.Assembly != drivenAssembly);
        }
